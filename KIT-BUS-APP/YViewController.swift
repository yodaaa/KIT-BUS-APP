...
//        self.timetable.layer.shadowColor = UIColor.black.cgColor
        ...
        timecardview.frame = CGRect(x: 0, y: 10, width: Int(screenSize.width - screenRatioWidth*20) , height: Int(timetable.bounds.size.height)-50)
        Logger.debugLog(timetable.bounds.size.height)
        Logger.debugLog(screenSize.height)
